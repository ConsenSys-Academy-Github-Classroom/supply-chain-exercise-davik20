...
contract SupplyChain {
  ...
  modifier forSale(uint sku) {
    ...
    require(item.state == State.ForSale, "This item is not for sale");
  ...
  // modifier sold(uint _sku) 
   modifier sold(uint sku) {
    Item memory item = items[sku];
    require(item.state == State.Sold, "This item hasnt been sold yet");
  ...
  // modifier shipped(uint _sku) 
   modifier shipped(uint sku) {
    Item memory item = items[sku];
    require(item.state == State.Shipped, "This item hasnt been sold yet");
  ...
  function buyItem(uint sku)payable public forSale(sku)  paidEnough(items[sku].price) checkValue(sku) {
      ...
      Item  memory itemBought = items[sku];
      
    
      itemBought.buyer = payable(msg.sender);
      itemBought.state = State.Sold;
      items[sku] = itemBought;
